(function() {
  define( [ "utils" ], function( utils ) {
    ...
    var Menu = function( pm ) {
      ...
      buttonManager.add( "import-json", "import-json-btnxxx", {
        click: function() {
      ...
      // XXXX
      buttonManager.add( "import-json-two", "import-json-btn", {
        click: function() {
        	
     			// begin axaj call instead	    	
        	//var server = 'http://127.0.0.1/elearning/vi2/_attachments/';
        	var server = '/vi-lab/';
     			
          var dataString = '433';//$("#import-json-select option:selected").val();
          if ( dataString ) { 
          		...
          		$.ajax({
    						type: "GET",
    						dataType: "json",
    						url: '/json/videos',//server+'data-slides.json',
    						success: function(data){  alert('got data')
    							$.each(data, function(j, val){ 
    								if(val.id == dataString){
    									//
    									var butter = '{"template":"basic","title":"'+dataString+'","guid":"AA41AB3B-D145-477E-A264-3B42701F1E85", "project": {"targets":[{"id":0,"name":"Area1"},{"id":1,"name":"pop-container"}],"media":[ {"id":"Media1","name":"Media01327337635028","url":"'+val.video+'","target":"main","duration":4829.205,"tracks":[';
							  
    									// fetch slides
											$.each(val.slides, function(i, val){ 
												if(i % 2 == 0 ){ 
													butter1 += '{"id":"TrackEvent'+i+'","type":"image","popcornOptions": {"start":'+this.starttime+',"end":'+Number(Number(this.starttime) + Number(this.duration)) +',"href":"","src":"'+server+'slides/'+dataString+'/'+this.img+'","text":"","target":"Area1"}, "track":"Track1327337639244","name":"Track1327337639'+Math.ceil(Math.random()*1000)+'"},';		
												}else{
													butter2 += '{"id":"TrackEvent'+i+'","type":"image","popcornOptions": {"start":'+this.starttime+',"end":'+ Number(Number(this.starttime) + Number(this.duration)) +',"href":"","src":"'+server+'slides/'+dataString+'/'+this.img+'","text":"","target":"Area1"}, "track":"Track1327337639255","name":"Track1327337639'+Math.ceil(Math.random()*1000)+'"},';		
												}
											});
											// fetch links
											$.each(val.links, function(i, val){
														butter3 += '{"id":"TrackEventA'+i+'","type":"pop","popcornOptions":{"start":'+Number(deci2seconds(this.start))+',"end":'+(Number(deci2seconds(this.start))+Number(this.duration))+',"exit":"2.5","text":"'+this.id+'", "link":"'+this.text+'","target":"pop-container", "left":"'+this.x+'%", "top":"'+this.y+'%"},"track":"Track1327357889566","name":"Track1327357889'+Math.ceil(Math.random()*1000)+'"},';		
											});
											
											// concatinate popcorn.js-json 
              				...
              				pm.importProject( data, document.getElementById( 'timeline-media-input-box' ).value );
								...
								error: function(e){ alert('error at json import'+JSON.stringify(e)); },
							});
          }
        }
      }); //import-json
